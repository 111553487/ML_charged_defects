...
# -*- coding: utf-8 -*-
"""
完全GPU加速的PyTorch Geometric CGCNN训练脚本 - 早期电荷集成 + 选择性池化版本
预测氧空位形成能
...
使用方法:
    python train_cgcnn_pyg_gpu_charge_early_selective.py --epochs 250 --batch-size 32
    python train_cgcnn_pyg_gpu_charge_early_selective.py --epochs 1000 --batch-size 500  # 完整训练
    python train_cgcnn_pyg_gpu_charge_early_selective.py --use-charge False  # 不使用电荷信息（消融研究）
"""
...
from cgcnn_pyg.model_pyg_charge_early_selective import CGCNNLightningChargeEarlySelective
    ...
    def on_train_end(self, trainer, pl_module):
        ...
        if len(self.val_predictions) > 0:
            ...
            val_results = {
                ...
                'model': 'CGCNN with early charge integration and selective pooling'
            }
...
def main():
    parser = argparse.ArgumentParser(
        description="Train PyG CGCNN model with early charge integration and selective pooling"
    )
    ...
    parser.add_argument("--num-cnn", type=int, default=5,
                        help="Number of convolutional layers")
    parser.add_argument("--dropout-ratio", type=float, default=0.3,
                        help="Dropout ratio")
    ...
    output_dir = Path(args.output_dir) / f"pyg_gpu_charge_early_selective_{timestamp}"
    output_dir.mkdir(parents=True, exist_ok=True)
    checkpoint_dir = output_dir / "checkpoints"
    checkpoint_dir.mkdir(exist_ok=True)
    
    print("=" * 80)
    print("CGCNN训练脚本 - 早期电荷集成 + 选择性池化")
    print("=" * 80)
    ...
    split_info = {
        'train_indices': train_indices.tolist() if isinstance(train_indices, torch.Tensor) else train_indices,
        'val_indices': val_indices.tolist() if isinstance(val_indices, torch.Tensor) else val_indices,
        'test_indices': test_indices.tolist() if isinstance(test_indices, torch.Tensor) else test_indices,
    ...
    print(f"  - CNN层数: {args.num_cnn}")
    print(f"  - Dropout比率: {args.dropout_ratio}")
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        filename="cgcnn-{epoch:03d}-{val_loss:.4f}",
        monitor="val_loss",
        mode="min",
        save_top_k=3,
        verbose=True
