...
class CGCNNPyGChargeEarlySelective(nn.Module):
    """
    ...
    def forward(self, data: Batch) -> torch.Tensor:
        ...
        if hasattr(data, 'target_site_indices') and data.target_site_indices is not None:
            # target_site_indices 现在包含全局索引（由collate_fn处理）
            # 确保索引在有效范围内
...
class CGCNNLightningChargeEarlySelective(pl.LightningModule):
    ...
    def __init__(
        ...
        # 模型
        self.model = CGCNNPyGChargeEarlySelective(
    ...
    def configure_optimizers(self):
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
            optimizer,
            mode='min',              # 验证损失越小越好
            factor=self.reduce_lr_factor,      # 学习率衰减因子
            patience=self.reduce_lr_patience,  # 容忍的 epoch 数
            threshold=self.reduce_lr_threshold, # 判断改进的阈值
            min_lr=1e-6              # 最小学习率
        )
...
if __name__ == "__main__":
    ...
    # 创建模型
    model = CGCNNPyGChargeEarlySelective(
        num_atom_features=num_atom_features,
        num_bond_features=num_bond_features,
        embedding_dim=64,
        hidden_dim=128,
